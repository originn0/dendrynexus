  ...
  Game.prototype._checkOptionTitles = function(callback) {
    ...
    _.every(_.values(this.scenes), function(scene) {
      return _.every(scene.options, function(option) {
        if (option.id.substr(0, 1) === '#') {
          ...
          assert(targetScene !== undefined);

          var isChoiceText =
            targetScene.tags &&
            targetScene.tags.indexOf('choice_text') >= 0;

          if (!option.title && !targetScene.title && !isChoiceText) {
            err = new Error(
              'Option refers to scene "' +
              targetScene.id +
    ...
    _.every(_.keys(tagsUsedAsOptions), function(tag) {
      ...
      if (sceneIds.length === 0) {
        err = new Error(
          'We have an option pointing at a tag "' +
          tag +
      ...
      return _.every(sceneIds, function(sceneId) {
        ...
        var isChoiceText =
          scene.tags &&
          scene.tags.indexOf('choice_text') >= 0;

        if (!scene.title && !isChoiceText) {
          err = new Error(
            'Scene "' +
            sceneId +
            '" can be selected as an option for tag "' +
            tag +
            '" but has no title.'
          );
          return false;
