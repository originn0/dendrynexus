...
(function() {
  ...
  var validateFrequency = function(object, propertyName, callback) {
    ...
    // If no semicolon and no condition, preserve original behavior
    if (val.indexOf(';') < 0 && val.indexOf(' if ') < 0) {
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      var name = frequency.toLowerCase();
      ...
      if (/^(always|\*)$/.test(name)) {
        ...
        // Validate numeric frequency
        var tempObject = {};
        tempObject[propertyName] = frequency;

        _frequencyFloatValidator(tempObject, propertyName, function(err, value) {
          if (err) {
            hasError = true;
            error = properr(object, propertyName, err.message);
          } else {
    ...
    if (hasError) return callback(error);
    return callback(null, result);
  };
  ...
  var validateConditionalImage = function(object, propertyName, callback) {
    assert(callback !== undefined, 'Callback required (wrong number of args?)');
    var val = propval(object, propertyName).trim();

    // If no semicolon, treat as plain image URL (original behavior)
    if (val.indexOf(';') < 0 && val.indexOf(' if ') < 0) {
      return callback(null, val);
    }
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      if (parts.length === 2) {
        validatePredicate(parts[1], null, function(err, predicate) {
          ...
          result.push({ image: parts[0].trim(), predicate: predicate });
        });
      } else {
        result.push({ image: parts[0].trim() }); // fallback, no condition
      }
    }
  ...
  module.exports = {
    ...
    validateConditionalImage: validateConditionalImage
  };
}());
