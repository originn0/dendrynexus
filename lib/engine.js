...
(function() {
  ...
  DendryEngine.prototype.__getChoiceDisplayData = function(choicesSelected) {
    ...
    for (var i = 0; i < choicesSelected.length; ++i) {
      ...
      var isChoiceText = choiceScene.isChoiceText === true;
  ...
  DendryEngine.prototype._compileChoices = function(scene) {
    ...
    if (options !== undefined) {
      var choiceIds =
        this.__getChoiceIdsFromOptions(options);

      choiceIds =
        this.__filterViewable(choiceIds);

      var validChoiceData =
        this.__getChoiceSelectionData(choiceIds);
      ...
      for (var i = 0; i < validChoiceData.length; ++i) {
        ...
        var choiceScene =
          this.game.scenes[choiceData.id];

        if (choiceScene.isChoiceText === true) {
          displayOnlyData.push(choiceData);
        } else {
      ...
      var minChoices =
        scene.minChoices || null;

      var maxChoices =
        scene.maxChoices || null;

      selectableData =
        this.__filterByPriority(
          selectableData,
          minChoices,
          maxChoices
        );
      ...
      validChoiceData =
        displayOnlyData.concat(selectableData);
      ...
      var data =
        this.__getChoiceDisplayData(validChoiceData);
    ...
    if (numChoosable === 0) {
      var root =
        this.state.rootSceneId;

      if (root !== this.state.sceneId) {
        var rootSceneChoose =
          this.game.scenes[root].chooseIf;
