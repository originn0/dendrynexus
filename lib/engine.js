...
(function() {
  ...
  UserInterface.prototype.displayContent = function(paragraphs, faceImage, wideImage, bannerImage) {};
  ...
  // helper: resolves a faceImage/cardImage value (string or condition array)
  DendryEngine.prototype._resolveImage = function(imageVal) {
    if (!imageVal) return null;
    if (typeof imageVal === 'string') return imageVal;
    // array of {image, predicate?}
    for (var i = 0; i < imageVal.length; i++) {
      var entry = imageVal[i];
      if (!entry.predicate || this._runPredicate(entry.predicate)) {
        return entry.image;
      }
    }
    return null;
    ...
    if (scene.isHand) {
      ...
      for (var c of choices) {
        ...
        if (choiceScene.isDeck) {
          ...
          c.isDeck = true;
          c.image = this._resolveImage(choiceScene.cardImage);
          decks.push(c);
      ...
      if (scene.maxCards > 0) {
          var resolvedHand = currentHand.map(function(card) {
            return {id: card.id, title: card.title, image: this._resolveImage(card.image)};
          }, this);
          this.ui.displayHand(resolvedHand, scene.maxCards);
      }
  ...
  DendryEngine.prototype.displaySceneContent = function(restorePage) {
    ...
    var faceImage = this._resolveImage(scene.faceImage);
    var bannerImage = this._resolveImage(scene.bannerImage);
    var wideImage =this._resolveImage(scene.wideImage);
    ...
    if (restorePage) {
      this.ui.newPage();
      this.ui.displayContent(this.state.tempCurrentContent, faceImage, wideImage, bannerImage);
    ...
    if (scene.content !== undefined && !restorePage) {
      ...
      this.ui.displayContent(displayContent, faceImage, wideImage, bannerImage);
    }
  ...
  DendryEngine.prototype.drawCard = function(deckId) {
    ...
    // display the hand
    var resolvedHand = this.state.currentHands[currentSceneId].map(function(card) {
      return {id: card.id, title: card.title, image: this._resolveImage(card.image)};
    }, this);
    this.ui.displayHand(resolvedHand, scene.maxCards);
    return card;
  ...
  DendryEngine.prototype.__getChoiceSelectionData = function(idToInfoMap) {
    ...
    for (var id in idToInfoMap) {
      ...
      // something
      if (Array.isArray(optionInfo.frequency)) {
        var frequencyConditions = optionInfo.frequency;
        var resolvedFrequency = 100;

        for (var i = 0; i < frequencyConditions.length; i++) {
          var frequencyCondition = frequencyConditions[i];

          if (!frequencyCondition.predicate ||
              this._runPredicate(frequencyCondition.predicate)) {
            resolvedFrequency = frequencyCondition.frequency;
            break;
          }
        }

        optionInfo.frequency = resolvedFrequency;
      }
  ...
  DendryEngine.prototype.__getChoiceDisplayData = function(choicesSelected) {
    ...
    for (var i = 0; i < choicesSelected.length; ++i) {
      ...
      var isChoiceText =
        choiceScene.tags &&
        choiceScene.tags.indexOf('choice_text') >= 0;

      if (isChoiceText) {
        var displayOnlyChoice = {
          id: choice.id,
          canChoose: false,
          isChoiceText: true
        };
        ...
        if (choiceScene.tags) {
          for (var t = 0; t < choiceScene.tags.length; ++t) {
            if (choiceScene.tags[t].indexOf('divider_') === 0) {
              displayOnlyChoice.dividerColor =
                '#' + choiceScene.tags[t].substring(8);
              break;
            }
          }
        }

        var displayTitle = choice.title || choiceScene.title;

        if (displayTitle !== undefined) {
          displayOnlyChoice.title =
            this._makeDisplayContent(displayTitle, false);
        }

        var displaySubtitle =
          choice.subtitle || choiceScene.subtitle;

        if (displaySubtitle !== undefined) {
          displayOnlyChoice.subtitle =
            this._makeDisplayContent(displaySubtitle, false);
        }

        if (choiceScene.content !== undefined) {
          displayOnlyChoice.content =
            this._makeDisplayContent(choiceScene.content, true);
        }

        choiceOutput.push(displayOnlyChoice);
        continue;
      }
      ...
      if (canChoose && choiceScene.chooseIf) {
        canChoose = this._runPredicate(
          choiceScene.chooseIf,
          true
        );
      }
      ...
      if (subtitle) {
        finalChoice.subtitle =
          this._makeDisplayContent(subtitle, false);
      }
      ...
      if (
        ...
        finalChoice.checkQuality =
          choiceScene.checkQuality;

        finalChoice.successProb =
          successProb;

        finalChoice.difficulty =
          displayDifficulty(successProb);
  ...
  DendryEngine.prototype._compileChoices = function(scene) {
    ...
    if (options !== undefined) {
      ...
      for (var i = 0; i < validChoiceData.length; ++i) {
        var choiceData = validChoiceData[i];
        var choiceScene = this.game.scenes[choiceData.id];

        if (
          !choiceScene.tags ||
          choiceScene.tags.indexOf('choice_text') < 0
        ) {
          selectableData.push(choiceData);
        }
      }
      ...
      var selectedSelectableData = this.__filterByPriority(
        selectableData,
        minChoices,
        maxChoices
      );
      ...
      for (var j = 0; j < selectedSelectableData.length; ++j) {
        selectedIds[selectedSelectableData[j].id] = true;
      }

      var finalChoiceData = [];

      for (var k = 0; k < validChoiceData.length; ++k) {
        var originalChoice = validChoiceData[k];
        var originalScene = this.game.scenes[originalChoice.id];

        var isChoiceText =
          originalScene.tags &&
          originalScene.tags.indexOf('choice_text') >= 0;

        if (isChoiceText || selectedIds[originalChoice.id]) {
          finalChoiceData.push(originalChoice);
        }
      }

      var data = this.__getChoiceDisplayData(finalChoiceData);
    ...
    if (numChoosable === 0) {
      ...
      if (root !== this.state.sceneId) {
        ...
        if (
          !rootSceneChoose ||
          this._runPredicate(rootSceneChoose, true)
        ) {
          choiceOutput.push({
            id: root,
            title: 'Continue...',
